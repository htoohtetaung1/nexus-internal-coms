...
import { Filter, MessageCircle, Heart, Share2, MoreHorizontal, ShieldAlert, Image as ImageIcon, Send, X, Crop, Search } from 'lucide-react';
...
const INITIAL_POSTS: Post[] = [
  ...
  },
  {
    ...
    content: 'Huge shoutout to the EMEA team for closing the TechGlobal deal! This puts us at 110% of our Q3 quota. Amazing work everyone! 🚀 #SalesWin #Q3Goals',
...
const Feed: React.FC = () => {
  ...
  const filteredPosts = posts.filter(post => {
    // 1. Department Filter
    const matchesDepartment = filter === Department.ALL || post.department === filter || post.department === Department.ALL;
    
    // 2. Search Filter (Content or Tags)
    ...
    if (searchTerm.trim()) {
      ...
      matchesSearch = matchesContent || !!matchesTags;
    }
  ...
  const handleCreatePost = () => {
    ...
    const extractedTags = newPostContent.match(/#[\w]+/g) || [];
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <div className="relative w-full sm:w-auto">
                ...
                <input 
                    ...
                    className="w-full sm:w-64 pl-9 pr-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition-colors text-sm text-slate-700 dark:text-slate-200 shadow-sm placeholder:text-slate-400"
                />
                {searchTerm && (
                  <button 
            ...
            <div className="relative w-full sm:w-auto">
                <div className="flex items-center gap-2 bg-white dark:bg-slate-800 px-4 py-2 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 transition-colors w-full">
                    <Filter size={16} className="text-slate-500 dark:text-slate-400 shrink-0" />
                    <select 
                        value={filter}
                        onChange={(e) => setFilter(e.target.value as Department)}
                        className="bg-transparent border-none focus:ring-0 text-sm font-medium text-slate-700 dark:text-slate-200 outline-none w-full cursor-pointer"
                    >
        ...
        <div className="flex gap-4">
            ...
            <div className="flex-1">
                <textarea 
                    ...
                    placeholder="What's happening in your department? Use #hashtags to tag topics." 
      ...
      <div className="space-y-6">
        {filteredPosts.length > 0 ? (
            ...
            <div key={post.id} className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden hover:shadow-md transition-all duration-200">
                ...
                <p className="text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-line mb-3">
            ...
            <div className="text-center py-12 text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-900/50 rounded-xl border border-dashed border-slate-300 dark:border-slate-800">
